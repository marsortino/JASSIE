...
import os
import sys
from lib.ext_compton import ExternalCompton as ExternalCompton_v1
...
class synchrotron:
    ...
    def flux_ssc_ssa(self, blocklist, nu, f_debug):
        """
        Computes the flux in the case in which the self compton is present.
        ...
        # DEBUG
        if os.path.exists('output_ssc_debug.txt'):
            f_debug = open('output_ssc_debug.txt', 'a')
        else:
            f_debug = open('output_ssc_debug.txt', 'x')
        ...
        for block in blocklist:
            ...
            f_debug.write('gamma_absorption_self: ' + str(gamma_absorption_self) + '\n')
            synchro_flux = Synchrotron(block.blob, ssa = True).sed_flux(nu)*gamma_absorption_self
            f_debug.write('synchrotron flux:\n')
            for i in range(0, len(synchro_flux)):
                f_debug.write(str(synchro_flux[i]))
                if i == 4:
                    f_debug.write('\n')

            ssc = SynchrotronSelfCompton(block.blob).sed_flux(nu)*gamma_absorption_self
            f_debug.write('ssc flux:\n')
            for i in range(0, len(ssc)):
                f_debug.write(str(ssc[i]))
                if i == 4:
                    f_debug.write('\n')
            ...
            for index in block.distance_order:
                ...
                flux_post_interaction *= gamma_absorption
                # The resulting flux is the one used to compute the SSC
                ssc_sed = self.synchro_self_compton(nu, flux_post_interaction, block.blob, scattering_blob, *scattering_blob.n_e.parameters, f_debug)
        ...
        for block in blocklist:
            ...
            if block.EC:
                for target in targetlist:
                    ...
                    ec_sed = ExternalCompton_v1(block, target).sed_flux_from_disk(nu, self.mu_s)
...
def warning_on_divide_by_zero(message, filename, lineno):
    # Log detailed information about the warning
    print(f"Warning: {message}")
    print(f"Occurred in file: {filename}, line: {lineno}")
    traceback.print_stack()

def warning_on_sqrt(message, lineno):
    # Log detailed information about the warning
    print(f"Warning: {message}")
    print(f"Occurred in line: {lineno}")
    traceback.print_stack()

def warning_on_add(message, lineno):
    # Log detailed information about the warning
    print(f"Warning: {message}")
    print(f"Occurred in line: {lineno}")
    traceback.print_stack()
